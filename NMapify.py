...
temp1 = []
for x in f:
	new3 = re.findall("\<port protocol\=.*. portid\=.*.\>\<state state\=\"open\" reason\=.*. reason_ttl\=.*.\/\>\<service name\=\".*.\" product\=|<address.*./>",str(x))
	new4 = re.sub("\[\'<address addr=\"|\" addrtype=\"ipv4\" />\'\]|\[\]|\<port protocol=\"(.*?)\" portid=\"|\>|\<state state=\"open\"|reason\=\"(.*?)\"|product\=|reason_ttl=\"(.*?)\"|\<service name\= \|\/|\"|\'|\[|\]","",str(new3))
	new5 = re.sub("\/\<service name\=",":",str(new4))
